...
export async function POST(req) {
    try {
        ...
        const PROMPT = `Generate a comprehensive and detailed study material for ${topic} designed for ${studyType} at a ${difficultyLevel} difficulty level. The study material should be extensive, covering all essential aspects of the topic to ensure thorough learning. Include the following in JSON format: 1. Course Title (as course_title make sure to use a good name) 2. Diffculty Level (difficulty)   3. **Course Summary**: A detailed overview of the course (50-100 words with key as CourseSummary), including its objectives, target audience, and key learning outcomes.  4. **List of Chapters**: A minimum of 8 chapters, each with:- **Chapter Title**: A clear and descriptive title.- **Chapter Summary**: A detailed summary of the chapter’s content (at least 100 words per chapter).- **Topic List**: A list of at least 5 specific topics covered in the chapter, each with a brief description (2-3 sentences per topic). 5. **Additional Resources**: Suggest at least 3 resources (e.g., books, websites, or videos) relevant to the course, with a brief description of each.`
